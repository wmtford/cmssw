...
private:
  ...
  void makeMap(const edm::Event& theEvent, const vstring trackerContainers);
...
//     trackerContainers.push_back("TrackerHitsPixelEndcapHighTof");
  ...
  makeMap(iEvent, trackerContainers);
...
void ClusterNtuplizer::makeMap(const edm::Event& theEvent, const vstring trackerContainers) {
  // Step A: Get Inputs
  ...
  for(auto const& trackerContainer : trackerContainers) {
    ...
    if (theEvent.getByLabel(tag_cf, cf_simhit)) {
      ...
      for (MixCollection<PSimHit>::iterator isim = thisContainerHits->begin(); isim != thisContainerHits->end(); isim++) {
	...
	Nhits++;
      ...
      std::cout << "simHits from crossing frames; map size = " << SimHitCollMap.size()
		<< ", Hit count = " << Nhits << ", " << sizeof(SimHitCollMap) << " bytes" << std::endl;
    } else {
	...
	Nhits++;
      ...
      if (printOut_ > 0) std::cout << "simHits from prompt collections; map size = " << SimHitCollMap.size()
				   << ", Hit count = " << Nhits << ", " << sizeof(SimHitCollMap) << " bytes" << std::endl;
    }
  }
}
